# OpenClaw 固定 IP 部署后 `device pairing required` 处理

## 问题现象

在服务器部署 OpenClaw，并通过固定 IP 访问：
...
sudo bash deploy_openclaw/deploy_openclaw.sh --approve-device 6333d195-1642-4621-a399-e7bf935b54c8
```
...
```bash
cd ~/DevOpsClaw/deploy
sudo bash deploy_openclaw/deploy_openclaw.sh --list-devices
```
...
```bash
cd ~/DevOpsClaw/deploy
sudo bash deploy_openclaw/deploy_openclaw.sh --reapply-token-auth
```
...
- `deploy/deploy_openclaw/deploy_openclaw.sh`

在执行：

```bash
sudo bash deploy_openclaw/deploy_openclaw.sh --deploy
```
...
1. 服务器上运行的脚本还不是最新版本
2. 浏览器缓存了旧的设备身份
3. OpenClaw 当前版本对 token + HTTPS 的处理仍未完全绕过设备配对
...
# 批准某个配对请求
sudo bash deploy_openclaw/deploy_openclaw.sh --approve-device <requestId>

# 重写 token 直连配置
sudo bash deploy_openclaw/deploy_openclaw.sh --reapply-token-auth

# 查看 OpenClaw 当前状态
sudo bash deploy_openclaw/deploy_openclaw.sh --status