...
2.1 Agent 容器
检查项	命令	结果
容器运行状态	docker ps | grep devopsagent-agent	✅ Up (healthy)
Agent 版本	agent --version	✅ 2026.5.5
Gateway 状态	agent status	✅ 运行中，端口 18789
2.2 Jenkins 连通性
检查项	命令	结果
...
2.4 已安装 Skill
Skill	路径	状态
jenkins	/home/node/.agent/workspace/skills/jenkins/	✅ ready
capability-evolver-pro	/home/node/.agent/workspace/skills/capability-evolver-pro/	✅ ready
n8n	/home/node/.agent/workspace/skills/n8n/	△ needs setup
tavily	/home/node/.agent/workspace/skills/tavily/	✅ ready
三、关键配置速查
...
Config validation failed: skills	workspace skill 不走 config set	直接用环境变量，不用 agent config set
Unknown command: agent jenkins	jenkins 是 workspace skill 不是 CLI 插件	用 node .../jenkins.mjs 直接调用
五、网络地址对照表
场景	Jenkins	GitLab
