...
按照最小权限原则，不要放在 /root 下，而是为 Agent 创建独立目录和用户映射：
bash
Copy
# 创建专用目录
sudo mkdir -p /home/agent/workspace

# Agent 容器内以 node 用户（UID 1000）运行
...
export AGENT_GATEWAY_TOKEN=$(tr -dc A-Za-z0-9 < /dev/urandom | head -c 64)
echo "你的 Gateway Token 是: $AGENT_GATEWAY_TOKEN"
# ⚠️ 请妥善保存此 Token，后续配对设备时需要用到

# 拉取官方镜像
docker pull ghcr.io/agent/agent:latest
四、以安全方式启动容器
...
docker run -d \
  ...
  -v /home/agent:/home/node/.agent \
  -e AGENT_GATEWAY_TOKEN="$AGENT_GATEWAY_TOKEN" \
  ghcr.io/agent/agent:latest \
  sh -c "node agent.mjs gateway --allow-unconfigured --bind loopback"
...
Copy
# 查看待批准的设备请求
docker exec -it agent node agent.mjs devices list

# 复制请求 UUID，然后批准它
docker exec -it agent node agent.mjs devices approve <request-uuid>
刷新浏览器页面，状态变为 Connected 即配对成功
运行初始化向导：
bash
Copy
docker exec -it agent node agent.mjs onboard
七、安装后的关键加固（必做）
1. 强制启用身份认证
确保 AGENT_GATEWAY_TOKEN 已设置（上文已配置），并在配置文件中确认：
bash
Copy
docker exec -it agent cat /home/node/.agent/agent.json
确认包含：
JSON
Copy
{
...
Copy
# 基础扫描
docker exec -it agent node agent.mjs security audit

# 深度扫描
docker exec -it agent node agent.mjs security audit --deep
4. 防火墙加固（建议）
...
Copy
# 查看当前日志大小
docker inspect --format='{{.LogPath}}' agent
ls -lh $(docker inspect --format='{{.LogPath}}' agent)
...
启动容器	docker start agent
重启容器	docker restart agent
查看日志	docker logs -f agent
进入容器	docker exec -it agent sh
更新镜像	docker pull ghcr.io/agent/agent:latest && docker restart agent
删除容器	docker rm -f agent
⚠️ 绝对禁止的操作
不要将 18789 端口直接映射到公网（如 -p 18789:18789）而不配置反向代理 + HTTPS + 强认证
不要在 Agent 中处理银行卡、密码、身份证等敏感信息 — API 密钥默认明文存储，存在泄露风险
...
未指定工作目录	🟡 中	添加 --workdir /home/node/agent
目录权限未显式设置	🟢 低	添加 chmod 750 /home/agent
...
(base) worker@master:~/software/AI$ docker exec agent env | grep AGENT
