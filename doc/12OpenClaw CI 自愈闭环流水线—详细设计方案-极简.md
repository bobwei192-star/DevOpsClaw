/home/zx/CICD/DevOpsClaw/doc/12OpenClaw CI 自愈闭环流水线—详细设计方案_极简.md OpenClaw CI 自愈闭环流水线 — 详细施工步骤（基于现有 Skill 整合版 v2.0）
版本: v2.0
日期: 2026-05-12
配套核心方案: 极简核心设计
目标: 基于 OpenClaw 已安装的 15 个 Skill + ClawHub 上的 GitLab Skill，实现 Jenkins 失败 → 自动抓日志 → AI 诊断 → 创建 fix 分支 → 触发重建 → 成功自动提 MR，最多重试 5 次，全程零人工。

一、前期准备
1.1 已确认可用的 Skill 清单
Skill	闭环中的角色	调用方式
jenkins	获取构建日志、触发构建	OpenClaw Agent 自动调用
...
配置环境变量：在 OpenClaw 的 Secret Manager 中设置 GITLAB_TOKEN 和 GITLAB_URL。
...
└── scripts/
    ...
    └── agent_wrapper.py              # 封装 openclaw agent 调用，返回结构化结果
2.2 SKILL.md（示例）
markdown
# ci-selfheal
...
OpenClaw Agent 可以通过命令行调用，并让它利用已安装的 Skill 去完成具体操作。
...
def ask_agent(instruction: str, expect_json: bool = True) -> Any:
    ...
    result = subprocess.run(
        ["openclaw", "agent", "--message", instruction, "--json"],
        check=True,
        capture_output=True,
        text=True,
    )
...
确保 OpenClaw 已配置好所有 Skill 的 API 密钥。
...
通过 openclaw agent --message "将今日自愈报告发送到 n8n" 定时发送摘要。
