...
# ============================================================
# 文件: bridge_v2.py
# 名称: OpenClaw Jenkins Self-Healing Bridge v2
# 版本: 2.0.0 (使用 OpenClaw CLI)
# 语言: Python 3.8+
# ============================================================
...
class Config:
    ...
    # OpenClaw 配置
    OPENCLAW_CLI = "docker exec openclaw node openclaw.mjs"
    DEFAULT_MODEL = "kimi-k2.5"  # 或 "deepseek-reasoner"
...
def ai_fix_with_cli(prompt: str, model: str = None) -> str:
    ...
    try:
        # 构建 OpenClaw CLI 命令
        # 使用 docker exec 在 openclaw 容器内执行
        cmd = [
        ...
        # 解析输出，提取 AI 回复
        # OpenClaw 输出格式包含元信息，需要提取实际的回复内容
...
def ai_fix(jenkinsfile: str, log_tail: str, round_num: int) -> str:
    ...
    for model in models_to_try:
        try:
            ...
            log_warn(f"模型 {model} 失败", error=str(e))
            last_error = e
            continue
...
def main():
    ...
    log_info("Bridge v2 启动中...", port=Config.PORT, dry_run=Config.DRY_RUN,
             max_retry=Config.MAX_RETRY)
    ...
    print(f"""
╔══════════════════════════════════════════════════════════════╗
║     OpenClaw Jenkins Self-Healing Bridge v2.0.0 (CLI Mode)   ║
╠══════════════════════════════════════════════════════════════╣
║  监听端口:    {str(Config.PORT).ljust(43)}║
║  OpenClaw:   使用 CLI (docker exec)                          ║
